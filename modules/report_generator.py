"""
DroidHunter — Report Generator Module
...
<title>DroidHunter — Security Report</title>
<style>
...
<header>
  <div>
    <div class="logo">👻 DroidHunter<span>Advanced Android Pentesting Framework</span></div>
  </div>
  <div class="meta">
    <div class="target">{target}</div>
    <div>Generated: {timestamp}</div>
    <div>Report ID: {report_id}</div>
    <div>Author: HexSecTeam | @hexsecteam</div>
  </div>
</header>
<main>
...
<footer>
  <p>DroidHunter &copy; 2026 | Author: <a href="https://www.instagram.com/hexsecteam">HexSecTeam</a> | For authorized use only</p>
</footer>
</body>
</html>
"""
...
def generate_html_report(data: dict, output: str = "droidhunter_report.html") -> str:
    """Generate a styled HTML report."""
    findings = data.get("findings", [])
    target   = data.get("target", "Unknown Target")
    ts       = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    report_id = f"DH-{int(time.time())}"
    rems = _remediations()
...
def generate_json_report(data: dict, output: str = "droidhunter_report.json") -> str:
    """Generate a structured JSON report."""
    report = {
        "tool": "DroidHunter",
        "author": "HexSecTeam | @hexsecteam",
...
def print_summary_table(data: dict):
    ...
    t = Table(title="[bold magenta]📋 DroidHunter — Findings Summary[/]",
